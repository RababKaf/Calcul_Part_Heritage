...
private Inter s;
	...
	@Override
    public User registerNewUserAccount(final UserDto accountDto,Office of) {
     ...
     // o.saveOffice(of); // Save the new office record
        s.saveOffice(of);
        user.setOffice(of);
    ...
    public List<User> getAllUserByenabled(boolean b) {
		System.err.println("*****************");
		return ur.findByEnabled(b);
	}

	public void changeUserState(Long id) {
	    User user = ur.getById(id);
		 boolean newEnabledState = !user.isEnabled();
	...
	public List<User> findByLastrnameContaining(String lastName) {
